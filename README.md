...
3. Set Appliku env vars (see [`appliku.yml`](appliku.yml)): `SLACK_SIGNING_SECRET`, `SLACK_BOT_TOKEN`, `SLACK_PROMPTS_CHANNEL_ID`, `SONGADAY_SLACK_PROMPT_TIMEZONE`, `PONDARBOR_ORIGIN`. Optional: `SLACK_SONGADAY_CHANNEL_ID` if submissions are not in the prompts channel; `SLACK_CLOSET_CHANNEL_ID` for the `#closet` channel where `/request` posts (must differ from Song-a-Day; invite the bot to that channel); `SLACK_CLOSET_NOTIFICATIONS_ENABLED=false` to disable Closet DMs; `SLACK_STAFF_NOTIFICATIONS_ENABLED=false` to disable staff DMs (new member approval, birth chart import, WhatIf proposals, contact messages); `SLACK_FRIEND_NOTIFICATIONS_ENABLED=false` to disable friend-request DMs; `SLACK_DM_THROTTLE_ENABLED=false` to disable the 24h proactive DM cap (default on); `SLACK_DM_THROTTLE_HOURS=24` to adjust the window. **Proactive ArborBot DMs are opt-in (default off):** members run `/pondarbor on` in Slack to receive closet, friend, and staff alerts; `/pondarbor off` to stop them. While opted out, notifications are queued and delivered when they opt back in. Proactive DMs are batched to at most one per user per window; overflow is delivered in digests when the window expires. Staff users (`is_staff`) bypass the 24h queue when opted in but must still run `/pondarbor on` like everyone else. Any slash command runs a site-wide digest flush (separate DMs) before the command response. Slash-requested DMs (`/closet`, `/loans`, `/request` match list), replies to a `#closet` request (I Do picker), and button-confirmation replies are not gated by opt-in and are not batched. Closet slash commands `/closet`, `/loans`, and `/request` require `im:write` and Interactivity URL `/api/v1/slack/interactions/` (see [`scripts/arborbot-slack-app-manifest.yaml`](scripts/arborbot-slack-app-manifest.yaml)). Staff DMs go to approved `is_staff` users with a linked Slack identity. Leave `VITE_AUTH0_SLACK_CONNECTION` unset.
