...
def disconnect_confirm(request):
    ...
    with transaction.atomic():
        ...
        pa = get_or_create_profile(a)
        pb = get_or_create_profile(b)
        pa.meal_crud_partner = None
        pb.meal_crud_partner = None
        pa.save(update_fields=["meal_crud_partner", "updated_at"])
        pb.save(update_fields=["meal_crud_partner", "updated_at"])
        req.status = MealPartnerDisconnectRequest.Status.COMPLETED
        req.save(update_fields=["status", "updated_at"])
